"""
Fetchr Coomer.su Scraper
Handles coomer.su and coomer.party user profiles and individual posts.
Uses the public JSON API identical to Kemono but at coomer.su domain.
"""
import aiohttp
import asyncio
...
class CoomerScraper(BaseScraper):
    DOMAINS = ["coomer.su", "coomer.party"]
    API_BASE = "https://coomer.su/api/v1"
    CDN_PRIMARY = "https://c3.coomer.su"
    CDN_FALLBACK = "https://c2.coomer.su"
    POSTS_PER_PAGE = 50
    ...
    async def scrape(self, url: str, cookies: str = "") -> List[ScrapedItem]:
        ...
        Scrape Coomer URLs.
        Handles:
        - https://coomer.su/{service}/user/{user_id}
        - https://coomer.su/{service}/user/{user_id}/post/{post_id}
        """
        try:
            ...
            print(f"Coomer: Found {len(items)} items from {url}")
            return items

        except Exception as e:
            print(f"Coomer scraper error: {e}")
            return []
    ...
    async def _scrape_single_post(
        ...
        """Fetch a single post from Coomer API."""
        try:
            ...
            print(f"Coomer: Error fetching single post: {e}")
            return []
    ...
    async def _scrape_user_posts(
        ...
        try:
            ...
            print(f"Coomer: Error fetching user posts: {e}")
            return items
    ...
    def _create_item_from_attachment(
        ...
        try:
            ...
            print(f"Coomer: Error creating attachment item: {e}")
            return None
    ...
    def _create_item_from_file(
        ...
        try:
            ...
            print(f"Coomer: Error creating file item: {e}")
            return None
